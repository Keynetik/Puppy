...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
   // NSString *link = [NSString stringWithFormat:@"http://graph.facebook.com/%@/picture?type=large", userData[@"id"]];
     NSString *link = [NSString stringWithFormat:@"http://graph.facebook.com/%@/picture?width=200&height=200", userData[@"id"]];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    UIImage * image = [UIImage imageNamed:@"1024_gm.png"];
    PFFile *fileThumbnail = [PFFile fileWithName:@"thumbnail.jpg" data:UIImageJPEGRepresentation(image, 0.9)];
    [fileThumbnail saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
     {
         if (error != nil) [ProgressHUD showError:NSLocalizedString(@"network_error", nil)];
     }];
  ...
  /*  user[PF_USER_FULLNAME] = twitter.screenName;
    ...
    user[@"isMale"] = @"true";
    user[@"desc"] = @"Hi all))) I am now with you !!!";
    user[@"dateVipEnd"] = [NSDate date];
    user[@"membervip"] = @"novip";
    user[@"points"] =  [NSNumber numberWithInt:0];
    user[@"nickname"] = twitter.screenName;
    user[@"username"] = twitter.userId; */
